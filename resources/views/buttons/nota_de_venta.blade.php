     ...
     <div class="dropdown-menu" x-placement="bottom-start"
         ...
         <a class="dropdown-item p-0 pl-2">
             <form method="POST" id="formdeletetx{{ $venta_id }}"
                 action="{{ route('ventas.destroy_nota', $venta_id) }}">
                 <input type="hidden" name="_token" value="{{ csrf_token() }}">
                 <input name="_method" type="hidden" value="DELETE">
                 <button type="submit" style="width: 100%"
                     onclick="FormDelete('tx{{ $venta_id }}','esta segur@ que desea eliminar esta venta',event)"
                     class="btn btn-ligth btn-sm m-1 text-left"><i class="fa fa-trash fa-1x"> </i> eliminar venta</button>
             </form>
         </a>
